...
def reservable_set_list_view(request):
    ...
    return Response(
        {"reservable_sets": reservable_sets},
...
def reservable_type_list_view(request, reservable_set_slug=None):
    ...
    return Response(d, template_name="reservations/reservable_type_list_view.html")


@api_view(["GET"])
@permission_classes((AllowAny,))
def reservable_resource_view(request, reservable_set_slug=None, reservable_type="room"):
    if reservable_set_slug is None:
        rs = None
    ...
    resources = (
        Resource.objects.filter(
            reservable__type=reservable_type, reservable__reservableset_set=rs
        )
        .distinct()
        .values("name", "id")
    )
    ...
    for i in reservables:
        ...
        for r in resources:
            try:
                n = NResources.objects.get(reservable=i, resource__id=r["id"]).n
            except:
                n = 0
            l.append(n)
    ...
    return Response(d, template_name="reservations/reservable_resource_view.html")
...
_ZOOMLEVELS = [
    # 4 hours, 5-min. intervals
    {
        ...
        "time_format": {"start": "H:i", "end": "H:i", "zoom_in": None, "zoom_out": "D"},
    },
...
def time_view(request, reservable_type="room", reservable_set_slug=None):
    ...
    if request.user.is_authenticated:
        if not hasattr(request.user, "reservations_profile"):
            ...
            profile = UserProfile.objects.create(user=request.user, sort_order=order)
        else:
    ...
    time_range, step, label_fmts = (
        zoomlevel["time_range"],
        zoomlevel["step"],
        zoomlevel["time_format"],
    )
    start = round_time(start_time, time_range)
    end = start + time_range
    time_busy_bitmap = [False] * int(
        math.ceil(time_range.total_seconds() / step.total_seconds())
    )
    full_res_list = []
    for r in reservables.order_by("slug"):
        reservations_list = []
        reservations = (
            r.reservation_set.filter(end__gte=start, start__lt=end)
            .order_by("start", "end")
            .iterator()
        )
        slot_start = start
        ...
        while slot_end <= end:
            ...
            while (
                len(running_reservations) > 0 and running_reservations[0][0] < slot_end
            ):
            ...
            owners_set = set()
            for res in expired_reservations + [i[1] for i in running_reservations]:
    ...
    for r, rlist in full_res_list:
        ...
        res_list.append({"reservable": r, "reservations": reservations_list})
    zoom_in_list = []
    zoom_out_list = []
    time_list = [{"start": i[1], "end": i[2], "span": i[3]} for i in pruned_times_list]
    for t in time_list:
        for l, zoom_step in [
            (zoom_in_list, zoomlevel_in["time_range"]),
            ...
            if zoom_step is not None:
                ...
                while last_end < t["end"]:
                    l[-1]["ranges"].append(
                        {"start": last_end, "end": last_end + zoom_step}
                    )
                    last_end += zoom_step
                l[-1]["last_end"] = last_end
    d = {
        "res_list": res_list,
        "zoom_in_list": zoom_in_list,
        "zoom_out_list": zoom_out_list,
        "time_list": time_list,
        "label_fmts": label_fmts,
        "step": step,
        "prev_time": start - time_range,
        "next_time": start + time_range,
        "zoom": zoom,
        "zoom_out": zoom + 1,
        "zoom_in": zoom - 1,
...
class ReservableFilter(filters.BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        for resource_slug, value in zip(resource_slugs, resource_values):
            queryset = queryset.filter(
                nresources__resource__slug=resource_slug, nresources__n__gte=value
            )
        return queryset
...
class FilteredReservableViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        response = super(FilteredReservableViewSet, self).list(request, *args, **kwargs)
...
class ReservationPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        u = request.user
        if not u:
            raise rest_framework.exceptions.PermissionDenied(detail=_("Please login"))
...
class ReservationSerializer(serializers.ModelSerializer):
    ...
    owners = serializers.PrimaryKeyRelatedField(
        many=True,
        label=Reservation.owners.field.verbose_name,
        read_only=True,
        # widget=autocomplete_light.ChoiceWidget(
        #    'UserAutocomplete',
        #    widget_attrs={'data-widget-maximum-values': 0}
...
class MyReservationsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return Reservation.objects.owned_by_user(self.request.user).order_by("-start")
...
class ReservationViewSet(viewsets.ModelViewSet):
    ...
    def send_email_notification(self, instance, created):
        ...
        reservables_email_representation = ",".join(
            instance.reservables.order_by("name").values_list("name", flat=True)
        )
        owners_email_representation = ",".join(
            [
                "{0} {1}".format(name, surname)
                for surname, name in instance.owners.order_by(
        ...
        if created:
            new_managers_emails = set(
                instance.owners.exclude(id=self.request.user.id).values_list(
                    "email", flat=True
                )
            )
        else:
            old_manager_ids = set(
                owner.id
                for owner in instance.old_owners
                if owner.id != self.request.user.id
            )
            new_manager_ids = set(
                instance.owners.exclude(id=self.request.user.id).values_list(
                    "id", flat=True
            ...
            new_managers_emails = set(
                instance.owners.filter(id__in=new_managers).values_list(
                    "email", flat=True
                )
            )
            persistent_managers_emails = set(
                instance.owners.filter(id__in=persistent_managers).values_list(
                    "email", flat=True
                )
            )
            removed_managers_emails = set(
                [
                    owner.email
                    for owner in instance.old_owners
                    if owner.id in removed_managers
                ]
            )

            old_reservables_email_representation = ",".join(
                [
                    reservable.name
                    for reservable in sorted(
                        instance.old_reservables, key=lambda reservable: reservable.name
                    )
                ]
            )
            old_owners_email_representation = ",".join(
                [
                    "{0} {1}".format(owner.first_name, owner.last_name)
                    for owner in sorted(
                        instance.old_owners,
                        key=lambda owner: "{0} {1}".format(
            ...
            if instance.reason != instance.old_reason:
                differences += 'Razlog: "{0}" namesto "{1}"\n'.format(
                    instance.reason, instance.old_reason
                )
            if instance.start != instance.old_start:
                differences += 'Začetek: "{0}" namesto "{1}"\n'.format(
                    instance.start, instance.old_start
                )
            if instance.end != instance.old_end:
                differences += 'Konec: "{0}" namesto "{1}"\n'.format(
                    instance.end, instance.old_end
                )
            if owners_email_representation != old_owners_email_representation:
                differences += 'Lastniki: "{0}" namesto "{1}"\n'.format(
                    owners_email_representation, old_owners_email_representation
                )
            if reservables_email_representation != old_reservables_email_representation:
                differences += 'Rezervirano: "{0}" namesto "{1}"\n'.format(
                    reservables_email_representation,
                    old_reservables_email_representation,
                )

            email_persistent_managers = (
                Reservation.PERSISTENT_OWNER_EMAIL_TEMPLATE.format(
                    differences, reservation_url
                )
            )
            send_mass_mail(
                [
                    ...
                    for recipient in persistent_managers_emails
                ]
            )

            email_removed_managers = Reservation.REMOVED_OWNER_EMAIL_TEMPLATE.format(
                instance.old_reason,
                instance.old_start,
                instance.old_end,
                old_reservables_email_representation,
                old_owners_email_representation,
                reservation_url,
            )
            send_mass_mail(
                (
                    (
                        "Odstranitev iz rezervacije",
                        email_removed_managers,
                        from_email,
                        [recipient],
                    )
                    for recipient in removed_managers_emails
                )
            )

        email_new_managers = Reservation.NEW_OWNER_EMAIL_TEMPLATE.format(
            instance.reason,
            instance.start,
            instance.end,
            reservables_email_representation,
            owners_email_representation,
            reservation_url,
        )
        send_mass_mail(
            (
                ("Nova rezervacija", email_new_managers, from_email, [recipient])
                for recipient in new_managers_emails
            )
        )
